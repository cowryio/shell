package main 

import (
	stone "../"
	"time"
)

var TXT = `
{
   "signatures": {
      "meta": "abcde",
      "ownership": "aa",
      "attributes": "abcde"
   },
   "meta":{
      "created_at":`+stone.IntToString(time.Now().Unix())+`,
      "stone_id": "`+stone.NewID()+`",
      "stone_type":"currency"
   },
   "ownership": {
      "type": "sole",
      "sole": {
      	  "address_id": "56c1f3d1f68daa4584000001"
      }
   },
   "embeds": [],
   "attributes": {
      "stuff": "stuff"
   }
}
`

func main() {

	// var sh = ""
	// var issuerPublicKey = "-----BEGIN RSA PUBLIC KEY-----\nMIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQC1CCgxMS5rro13G4frgt88bq00\ncdWj8r/VK3uUi7CK7Dq8dmbUU5SfqeuCzEg0EUIcaiSabndev5CfUVTPLKakmelA\nv7f8F0c3hTTaInZcZ9F9xuWdfklKVlU63fSEl14+qGddRyu/vs3wYHUgesZZG16R\nZcNmRfDqMd3XYkEz8QIDAQAB\n-----END RSA PUBLIC KEY-----\n"
	var issuerPrivateKey = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n"
	// var hostPrivateKey = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n"
	var meta = map[string]interface{}{
		"stone_id": stone.NewID(),
		"stone_type": "currency",
		"created_at": time.Now().Unix(),
	}

	sh, e := stone.Create(meta, issuerPrivateKey)
	stone.Println("CREATE: ", e)
	stone.Println(sh.Encode())
	// sh2, _ := stone.Load(sh.Encode())
	// stone.Println(sh2.Encode())
	// stone.Println("VERIFY: ", sh2.Verify("meta", pubKey))

	// sh, e := stone.Load(TXT)
	// stone.Println("LOAD:", e)
	// sig, err := sh.Sign("meta", issuerPrivateKey)
	// if err != nil {
	// 	stone.Println("SIGN META:", err)
	// 	return
	// }

	// sh.Signatures["meta"] = sig

	// sig, err = sh.Sign("ownership", hostPrivateKey)
	// if err != nil {
	// 	stone.Println("SIGN OWNERSHIP:", err)
	// 	return
	// }
	// sh.Signatures["ownership"] = sig
	
	// sig, err = sh.Sign("attributes", issuerPrivateKey)
	// if err != nil {
	// 	stone.Println("SIGN ATTRIBUTES:", err)
	// 	return
	// }
	// sh.Signatures["attributes"] = sig

	// sh2, _ := stone.Load(TXT)
	// sig, _ = sh.Sign("meta", issuerPrivateKey)
	// sh2.Signatures["meta"] = sig
	// sh.AddEmbed(sh2, issuerPrivateKey)

	// stone.Println("VERIFY: ", sh.Verify("meta", issuerPublicKey))
	// stone.Println("\n", sh.Encode())

	// manager := stone.Manager{}
	// aStone := manager.NewStone("currency");
	// aStone.AddTransaction(stone.GenesisTxn(1000));
	// fmt.Printf("%v\n", currencyStone)
	// stone.Println(aStone.JSON())
	// anoStone, err := stone.LoadJSON(TXT)
	// if err != nil {
	// 	stone.Println(err)
	// 	return
	// }
	// stone.Println(anoStone.Embeds[1].Embeds[0].Txns)
}

